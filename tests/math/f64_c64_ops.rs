...
mod f64_c64_ops_tests {
    ...
    fn setup() -> (
        ...
        let t1 = Tensor::new(&shape![2, 3, 4], (0..24).map(f64::from).collect()).unwrap();
        let t2 = t1.map_refs(|&x| Complex64::new(x, x));

        let m1 = Matrix::new(6, 4, (0..24).map(f64::from).collect()).unwrap();
    ...
    fn normalised() {
        ...
        assert_approx_eq!(
            f64,
            (t2_norm_l1_ans - t2_norm_l1).map(Complex64::abs).sum(),
            0.0,
            epsilon = 1e-15
        );
        assert_approx_eq!(
            f64,
            (t2_norm_l2_ans - t2_norm_l2).map(Complex64::abs).sum(),
            0.0,
            epsilon = 1e-15
        );
    }
    ...
    fn recip() {
        ...
        assert!(approx_eq!(
            Tensor<f64>,
            t1.map(|x| x + 1.0).recip(),
            ans1,
            epsilon = 1e-15
        ));
        assert!(approx_eq!(
            Tensor<Complex64>,
            t2.map(|x| x + Complex64::ONE).recip(),
            ans2,
            epsilon = 1e-15
        ));
        assert!(approx_eq!(
            Matrix<f64>,
            m1.map(|x| x + 1.0).recip(),
            ans3,
            epsilon = 1e-15
        ));
        assert!(approx_eq!(
            Matrix<Complex64>,
            m2.map(|x| x + Complex64::ONE).recip(),
            ans4,
            epsilon = 1e-15
        ));
    }
    ...
    fn sigmoid() {
        ...
        assert!(approx_eq!(
            Tensor<f64>,
            (t1.clone() - 12.0).sigmoid(),
            ans.clone().into_tensor().reshape(t1.shape()).unwrap(),
            epsilon = 1e-5
        ));
        assert!(approx_eq!(
            Matrix<f64>,
    ...
    fn softmax() {
        ...
        assert!(approx_eq!(
            Tensor<f64>,
            t1.softmax(),
            ans.clone().into_tensor().reshape(&t1_shape).unwrap(),
            epsilon = 1e-5
        ));
        assert!(approx_eq!(
            Matrix<f64>,
            m1.softmax(),
            ans.clone().into_matrix().reshape(m1_rows, m1_cols).unwrap(),
            epsilon = 1e-5
        ));
    }
