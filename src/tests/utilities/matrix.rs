...
mod matrix_utils_tests {
    ...
    use crate::utilities::matrix::{identity, pool_avg_mat, pool_max_mat, pool_min_mat, pool_sum_mat};
    ...
    fn invalid_concat() {
        ...
        assert_eq!(m1.concat(&m2, 0).unwrap_err(), TensorErrors::ShapesIncompatible);
    }
    ...
    fn invalid_reshape() {
        let m1 = Matrix::new(3, 3, (0..9).collect()).unwrap();
        assert_eq!(m1.reshape(2, 3).unwrap_err(), TensorErrors::ShapeSizeDoesNotMatch);
    }
    ...
    fn set_all_fail() {
        ...
        slice_mut.set_all(&Matrix::zeros(2, 1).unwrap()).unwrap();
    }
    ...
    fn concat_multithreaded() {
        let m1 = identity::<Complex64>(10).unwrap().reshape(20, 5).unwrap();
        let m2 = (0..500)
            .collect::<Matrix<_>>()
            .reshape(100, 5)
            .unwrap()
    ...
    fn invalid_slice_mut_index_ranges_invalid() {
        ...
        match err {
            TensorErrors::InvalidNonEmptyInterval { max: _, min: _ } => {},
        ...
        let err = m1.slice_mut(1..2, 1..1).unwrap_err();
        match err {
            TensorErrors::InvalidNonEmptyInterval { max: _, min: _ } => {},
            _ => panic!("Incorrect error")
        }

        let err = m1.slice_mut(1..1, 1..2).unwrap_err();
        match err {
            TensorErrors::InvalidNonEmptyInterval { max: _, min: _ } => {},
            _ => panic!("Incorrect error")
        }
